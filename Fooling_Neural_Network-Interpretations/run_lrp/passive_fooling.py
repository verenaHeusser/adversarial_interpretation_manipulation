        ...
        for i, data in enumerate(train_loader):
            ...
            if first_itr == False:
                ...
                if args.loss_type in ["topk", "center_mass"]:
                    ...
                    LRP_ori_ = net_ori.interpretation(
                        activation_output,
                ...
                if itr % loss_period == 0:  # print every 20 mini-batches
                    ...
                    np.save(
                        args.img_dir + args.img_name + str("/") + args.img_name + "_loss_c.npy",
                        loss_c_tr_list,
                    )
                    np.save(
                        args.img_dir + args.img_name + str("/") + args.img_name + "_loss_r.npy",
                        loss_r_tr_list,
                    )
                    np.save(
                        args.img_dir + args.img_name + str("/") + args.img_name + "_loss_t.npy",
                        loss_t_tr_list,
                    )
            ...
            if itr % args.eval_period == 0:
                ...
                with torch.no_grad():
                    ...
                    for j, data in enumerate(val_loader):
                        ...
                        visualize5(
                            R_Ts, R_inputs, R_Ts_keys, R_inputs_keys, inputs, itr, j, prediction
                        )
                ...
                if args.imagenet_accuracy:
                    test_acc1, test_acc5, loss_dict = test_accuracy(
                        net,
                        test_loader,
                        num_data=50000,
                        get_quanti="loss",
                        ...
                        net_ori=net_ori,
                        test_mode=True,
                        interpreter_list=interpreter_list,
                    )
                    test_acc1_list.append(test_acc1)
                    test_acc5_list.append(test_acc5)
                    
                    print('save stuff')
                    print(args.img_dir + args.img_name + str("/") + args.img_name + "_test_acc1.npy")

                    np.save(
                        args.img_dir + args.img_name + str("/") + args.img_name + "_test_acc1.npy",
                        test_acc1_list,
                    )
                    np.save(
                        args.img_dir + args.img_name + str("/") + args.img_name + "_test_acc5.npy",
                        test_acc5_list,
                    )
                    np.save(
                        args.img_dir + args.img_name + str("/") + args.img_name + "_loss_dict.npy",
                        loss_dict,
                    )
