...
# CUDA_VISIBLE_DEVICES=$GPU python3 $RUN --interpreter lrp_T --lrp_target_layer 34 --batch-size=2 --model VGG19 --lr 1e-5 --lambda_value 10 --loss_type location --img_name $EXP_NAME $FIXED_OPTION
...
CUDA_VISIBLE_DEVICES=$GPU python $RUN --interpreter lrp_T --lrp_target_layer 64 --model Densenet121 --lr 1e-5 --lambda_value 10 --loss_type location --img_name $EXP_NAME $FIXED_OPTION
